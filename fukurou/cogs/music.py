...
class MusicCog(commands.Cog):
    ...
    async def __play(self, ctx: ApplicationContext, *, track: str):
        ...
        if not player.is_connected(ctx.author.voice.channel):
            if await player.connect(ctx) == False:
                return
        ...
        elif music.origin == music.Playlist:
    ...
    async def __queue(self, ctx: ApplicationContext):
        ...
        embed = discord.Embed(title=":scroll: Queue [{}]".format(
            len(player.playlist.queue)), color=config.EMBED_COLOR)

        for counter, music in enumerate(list(player.playlist.queue)[:config.MAX_SONG_PRELOAD], start=1):
            if music.title is None:
                embed.add_field(name=f"{counter}.", value="[{}]({})".format(
                    music.webpage_url, music.webpage_url), inline=False)
            else:
                embed.add_field(name=f"{counter}.", value="[{}]({})".format(
                    music.title, music.webpage_url), inline=False)
    ...
    async def __history(self, ctx: ApplicationContext):
        ...
        # Embeds are limited to 25 fields
        if config.MAX_HISTORY_LENGTH > 25:
            config.MAX_HISTORY_LENGTH = 25

        embed = discord.Embed(title=":scroll: History [{}]".format(
            len(player.playlist.history)), color=config.EMBED_COLOR)

        for counter, music in enumerate(list(player.playlist.history)[:config.MAX_HISTORY_LENGTH], start=1):
            if music.title is None:
                embed.add_field(name=f"{counter}.", value="[{}]({})".format(
                    music.webpage_url, music.webpage_url), inline=False)
            else:
